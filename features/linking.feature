Feature: PlayFab <-> PlayReal Linking Workflow
  ...
  Scenario: Case #3A: Login to a PlayReal account that was already linked to another PlayFab account and choose to keep the current Content
    Given I have a non linked PlayFab account
  ...
  Scenario: Case #6A: Inconsistency: the PlayFab account points to the current PlayReal account but the PlayReal account points to ANOTHER PlayFab account: we choose to keep the current Content => same as #3A
  ...
  Scenario: Case #6B: Inconsistency: the PlayFab account points to the current PlayReal account but the PlayReal account points to ANOTHER PlayFab account: we choose to discard the current Content => same as #3B
    Given I have a linked PlayFab account to a badly linked PlayReal account
    And I choose to discard the current content
    When I run the workflow
    Then I should get two accounts
  ...
  Scenario: Case #9A: each is linked to another: ask the user which one to use: the user chooses the PlayReal account
    Given I have a linked PlayFab account
    And I have a PlayReal account linked to another PlayFab account
    And I choose to discard the current content
    When I run the workflow
  ...
  Scenario: Case #9B: each is linked to another: ask the user which one to use: the user chooses the other PlayReal account
    Given I have a linked PlayFab account
    And I have a PlayReal account linked to another PlayFab account
    And I choose to keep the current content
    When I run the workflow
